...
export default function PaymentsMade() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status?.toUpperCase()) {
      ...
      case 'VOIDED':
        return <span className="text-red-400 font-medium italic">VOIDED</span>;
      case 'DRAFT':
            ...
            <div className={`flex flex-col h-full ${selectedPayment ? 'w-full' : 'max-w-full mx-auto p-6'} animate-in fade-in duration-500`}>
              <div className="flex items-center justify-between p-4 border-b border-slate-200 bg-white sticky top-0 z-10 min-h-[73px] h-auto">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    className={cn(
                      "bg-sidebar hover:bg-sidebar/90 gap-1.5 h-9 font-semibold font-display",
                ...
                <div className="flex-1 overflow-y-auto scrollbar-hide">
                  <div className="divide-y divide-slate-100">
                    {filteredPayments.map((payment) => (
                      <div
                        key={payment.id}
                        className={`p-4 cursor-pointer transition-colors hover:bg-slate-50 ${selectedPayment.id === payment.id ? 'bg-sidebar/5 border-l-2 border-l-sidebar' : ''
                          }`}
                    ...
                    <div className="border rounded-lg overflow-hidden bg-white dark:bg-slate-900 mb-6">
                      <div className="overflow-x-auto">
                        <table className="w-full">
                          <thead className="bg-sidebar/5 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                            <tr>
                          ...
                          <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                            {paginatedItems.map((payment) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-4 text-sm font-medium text-sidebar hover:underline whitespace-nowrap">{getPaymentNumberString(payment.paymentNumber)}</td>
